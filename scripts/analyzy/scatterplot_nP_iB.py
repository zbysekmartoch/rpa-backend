...
def plot_for_each_product(df: pd.DataFrame, output_dir: str):
    ...
    for (product_id, product_name), grp in df.groupby(["product_id","product_name"], dropna=False):
        ...
        if len(model.params) > 1:
            ...
            print(f"  Only intercept could be estimated (iB has no variation).")
        ...
        if len(model.params) > 1:
            ...
            ax.plot(x_range, y_pred, color="#d70c0f", linewidth=0.5, linestyle="-",
                    label=f"OLS: iB = {model.params[0]:.2f} + {model.params[1]:.2f}Np")
        else:
        ...
        if len(model.params) > 1:
            ...
            stats_line = f"R² = {r2_text}, p-val(Np) {pval_text}\n"
            text_y = -0.35  # default position when regression is estimated
        else:
            stats_line = ""  # skip R² and p
            text_y = -0.35    # move text up when there’s no regression line
