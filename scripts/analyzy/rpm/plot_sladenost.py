...
def plot_for_each_product(df: pd.DataFrame, output_dir: str):
    ...
    for (product_id, product_name), grp in df.groupby(["product_id","product_name"], dropna=False):
        ...
        # Plot data
        ax.plot(x, y_on_par, label="S", color="#00469B")
        ...
        # Move axes to free space below for legend/caption
        # (removed second set_position — already handled above)
        ...
        # ---- Keep your original saving code exactly as is ----
...
def main():
    ...
    output_dir = os.path.join(work_dir, "img/sladenost")
